...
const Assignbookteachers = () => {
  ...
  const fetchAssignedBooks = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Users/Teachers/fetchassigned.php`,
        {
          method: "POST",
          headers: {
  ...
  const fetchTeacherName = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Users/Teachers/fetchteachername.php`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Users/Teachers/assignbooks.php`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="container mt-3 mb-3 p-3">
        <div className="row">
          <div className="col-md-12">
            <div className=" h4">
              Assign Books to <b>{teacherName}</b>
            </div>
            <hr></hr>
            <p className="fw-bold text-danger">
              *Click on Checkbox to add a Book{" "}
            </p>
