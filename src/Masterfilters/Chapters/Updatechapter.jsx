...
const Updatechapter = () => {
  ...
  const fetchChapterDetails = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Chapters/getchapter.php?chapter_id=${decodedId}`
      );
  ...
  const fetchSeries = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Series/fetchseries.php`
      );
      const data = await response.json();
      if (data.success) {
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Book/fetchbooks.php`
      );
      const data = await response.json();
      if (data.success) {
        setBooks(data.books);
  ...
  const fetchSections = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Sections/fetchsections.php`
      );
      const data = await response.json();
      if (data.success) {
  ...
  const fetchUnits = async () => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Units/fetchunits.php`
      );
      const data = await response.json();
      if (data.success) {
        setUnits(data.units);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Chapters/updatechapters.php?chapter_id=${decodedId}`,
        {
          method: "POST",
          body: formData,
        }
      ...
      >
        <div className="row">
          <div className="col-md-12">
            <div className="text-grey fw-bold h4">Update Chapter</div>
            <hr></hr>
            <form onSubmit={handleSubmit}>
              ...
              <label className="fw-bold">Chapter Description</label>
              <br />
              <textarea
                className="custom-input "
              ...
              <label className="fw-bold">Chapter Media </label>

              {existingPic && (
                <div style={{ position: "relative", display: "inline-block" }}>
                  <embed
                    src={`${config.apiBaseUrl}/admin/fullmarks-server/uploads/chapter_content/${existingPic}`}
                    alt="Page Image"
                    style={{ width: "100px" }}
                  />
                  <FaTimes
              ...
              {!existingPic && (
                <>
                  <span className="text-grey h6">Add New Chapter Media? </span>
                  <input
                    type="file"
                  ...
                  <span className=" text-danger">
                    (max size: 2 MB, jpg-jpeg-png allowed formats)
                  </span>
                  <br></br>
                </>
              )}
