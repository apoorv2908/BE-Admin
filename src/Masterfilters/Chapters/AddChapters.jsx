...
const AddChapters = () => {
  ...
  const fetchSections = async (classId, subjectId, seriesId, bookId) => {
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Sections/fetchsectionforunit.php?class_id=${classId}&subject_id=${subjectId}&series_id=${seriesId}&book_id=${bookId}`
      );
      const data = await response.json();
      if (data.success) {
  ...
  const fetchUnits = async (
    ...
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Book/fetchunitforchapter.php?class_id=${classId}&subject_id=${subjectId}&series_id=${seriesId}&book_id=${bookId}&section_id=${sectionId}`
      );
      const data = await response.json();
      if (data.success) {
        setUnits(data.units);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${config.apiBaseUrl}/fullmarks-server/Masterfilter/Chapters/addchapters.php`,
        {
          method: "POST",
          body: formData,
        }
      ...
      >
        <div className="row">
          <div className="col-md-12">
            <div className="text-grey fw-bold h4">Add Chapter</div>
            <hr></hr>
            <form onSubmit={handleSubmit}>
              ...
              <span
                ...
                style={{ fontSize: "13px" }}
              >
                Allowed Formats: jpg, jpeg, png, pdf (max size: 10MB){" "}
              </span>
              <br></br>
              <label className="fw-bold mt-1">Chapter Media </label>{" "}
              <input
                type="file"
